 ...
 *
 * ----------------------------------------------------------------------------------------------
 * Vendorise tel quel (aucune modification) depuis
 * waveshareteam/ESP32-S3-Touch-LCD-7, examples/Arduino/examples/10_lvgl_v9_demo/
 * esp_panel_board_custom_conf.h (recupere le 2026-07-28, commit le plus
 * recent au moment du fetch : bc51a84b, date 2026-07-25) -- broches/timings
 * deja documentes sans code driver dans BoardConfig7inch.h (materiel jamais
 * teste avant ce soir). Trouve via "#if __has_include("esp_panel_board_custom_conf.h")"
 * dans ESP32_Display_Panel (src/board/esp_panel_board_conf_internal.h) : ce
 * fichier remplace celui embarque par la librairie grace a "-I include"
 * (voir platformio.ini, env:waveshare-7inch-rgb), exactement le meme
 * mecanisme que lv_conf.h/LV_CONF_INCLUDE_SIMPLE deja utilise par ce projet.
 * ----------------------------------------------------------------------------------------------
 */
